...
window.addEventListener("load", function() {
    ...
    async function getPokemonOfTheDay() {
        let pokemonName = await getRandomPokemon();
        pokemon = await getPokemon(pokemonName.name);
    ...
    //This function applies an event handler to the getPokemonOfTheDay to load another random pokemon. Removing this from the function prevents PokemonDetails from loading
    ...
    //Adds functionality to the Show List of Pokemon button in the menu. It hides the feature pokemon and unhides the list of Pokemon - rather than having to reload
    ...
    async function fillPokemonListDetails(){
        ...
        //For each pokemon at the API end point we'll create a div, add the image, the images id will be set to the Pokemon's name, and the the Pokemon's name will be added as a heading
    ...
    //Calls the fillPokemonListDetailsFunction when the window is first loaded
